...
export const generatePowder = () => {
    if (powderCache.powder.length > 0 &&
...
export const getCredentials = async (email, password) => {
    ...
    if (credential) {
        ...
        console.log("Credentials with email " + email + " not found, trying to login...");
    }
    if (!password) {
        throw new Error("Password is required");
    }
    const loginOpt = {
        url: "https://rw1.drakor.id:443/login.php?version=3.6&key=941301bfce8fb05c314761ec7715a00e&hs=09fa00215116bd4884c4520cd27939ee&email=" +
            email +
            "&password=" +
            password,
    ...
    credential = (await response.json()).data;
    if (response.status === 200) {
        ...
        throw new Error(`Something went wrong with status ${response.status}, ${response.statusText}`);
    }
    return credential;
};
export const searchDrakor = async (query) => {
    console.log("searching for " + query);
    const url = "https://rw1.drakor.id:443/m_by_search.php?version=3.6&key=941301bfce8fb05c314761ec7715a00e&hs=09fa00215116bd4884c4520cd27939ee&page=1&q=" +
        query +
...
export const getDownloadLinks = async (showId, streamingId) => {
...
export const getFastDownliadLinks = async (streamingId) => {
...
export const getStreamingId = async (showId, episodeNumber) => {
